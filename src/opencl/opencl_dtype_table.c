...
void ocl_dtype_table_init(ocl_dtype_table_t* table, const ocl_device_extensions_t* exts) {
    if (!table || !exts) return;
    
    table->exts = *exts;

    int native_fp16 = exts->has_fp16;
    int native_fp64 = exts->has_fp64;
    int native_int64 = exts->has_int64;
    int native_int8 = exts->has_int8 && exts->has_8bit_storage;
    int native_int16 = exts->has_int16 && exts->has_16bit_storage;
